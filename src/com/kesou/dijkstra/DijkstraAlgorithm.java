...
class VisitedVertex {//已访问顶点集合
    public int[] already_arr;//记录各个顶点是否访问过，1表示访问过，0表示未访问过（会动态更新）
    public int[] pre_visited;//每个下标对应的值为前一个顶点下标，会动态更新
    public int[] dis;//记录出发顶点到其他所有顶点的距离，比如G为出发顶点，就会记录G到其他顶点的距离，会动态更新，求最短距离就会放进dis
    ...
    /**
     * 功能：判断index顶点是否被访问过，如过访问过则返回True否则False
     *
     * @param index
     * @return
     */
    ...
    /**
     * 继续选择并返回新的访问顶点，比如这里的G完后，就是A点作为新的访问节点(注意不是出发点)
     * @return
     */
